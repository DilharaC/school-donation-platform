// SchoolRegister.tsx (FULL) — modern UI + icons (no emojis) + success screen
...
const StepPill = ({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className={cx("text-sm font-extrabold", active ? "text-slate-800" : "text-slate-500")}>
        {label}
      </div>
    </div>
  );
...
export default function SchoolRegister() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-rose-50 via-[#F7F7FB] to-white">
      <div className="mx-auto max-w-5xl px-4 py-10">
        {/* top line */}
        <div className="mb-6 flex items-center justify-between">
          <Link to="/" className="inline-flex items-center gap-2 text-sm font-extrabold text-slate-700 hover:underline">
            <Icon name="arrowLeft" className="h-4 w-4" />
            Back to Home
          </Link>
          <div className="text-sm text-slate-600">
            Already registered?{" "}
            <Link className="font-extrabold text-rose-700 hover:underline" to="/school/login">
              Login
            </Link>
          </div>
        </div>
        ...
        {registered ? (
          <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
            <div className="p-8 text-center">
              ...
              <div className="mt-7 flex flex-col sm:flex-row gap-3 justify-center">
                <button
                  onClick={() => nav("/school/login?registered=1")}
          ...
          <>
            ...
            <div className="mt-6 bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
                    ...
                    <div className="rounded-3xl border border-slate-200 bg-slate-50/60 p-5">
                      ...
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        ...
                        <div className="md:col-span-2">
                          ...
                          <div className="font-extrabold text-slate-800">
                            Students: {form.student_count || "—"} • Facilities: {form.facilities || "—"} • Area:{" "}
                            {form.area_type || "—"} • Performance: {form.performance || "—"} • Prev Donations:{" "}
                            {form.prev_donations || "0"}
                          </div>
                        </div>
              ...
              {/* Footer (sticky style inside card) */}
