...
export default function AdminNotifications() {
  ...
  const markAllRead = async () => {
    try {
      await axios.post(
        NOTIF_READ_ALL,
        null,
        { withCredentials: true, params: { role: "admin" } }
      );
      ...
      // if user is on "Unread" tab, refresh to clear list
      if (tab === "unread") {
        setPage(1);
        setTimeout(() => fetchData(), 0);
      }
  ...
  const deleteNotif = async (id: string) => {
    ...
    try {
      await axios.delete(NOTIF_DELETE(id), { withCredentials: true, params: { role: "admin" } });
    ...
    <div className="px-6 py-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <div className="text-slate-500 text-sm mt-1">System updates and activity alerts</div>
        </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            ...
            <div className="flex-1 flex gap-2">
              <input
                ...
                placeholder="Search title/body/school_id/request_id..."
          ...
          rows.map((r) => {
            ...
            return (
              <div
                key={r.id}
                className={cx(
                  "px-5 py-4 border-t border-slate-100 transition",
