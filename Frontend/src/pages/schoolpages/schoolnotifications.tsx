...
/**
 * ✅ School-side: make message body more user-friendly.
 * - Prefer backend body
 * - Replace generic words if we have names/titles
 * - Never show IDs
 */
function buildPrettyBody(data: any) {
  const base = (data?.body || "").toString();
  const schoolName = data?.school_name || ""; // usually your own school
  const requestTitle = data?.request_title || "";

  if (base) {
    let out = base;

    // Replace "your school" / "The school" with name if available
    if (schoolName) {
      out = out.replace(/\byour school\b/gi, schoolName);
      out = out.replace(/\bthe school\b/gi, schoolName);
    }

    // Replace "Request #123" with request title if available
    if (requestTitle) {
  ...
  if (requestTitle) return `There is an update related to: ${requestTitle}.`;
...
export default function SchoolNotifications() {
  ...
  const deleteNotif = async (id: string) => {
    ...
    try {
      await axios.delete(NOTIF_DELETE(id), { withCredentials: true, params: { role: "school" } });
  ...
  return (
    <div className="px-6 py-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <div className="text-slate-500 text-sm mt-1">
            Updates about your campaigns, donations, approvals, and system alerts
          </div>
        </div>
        ...
        <div className="md:col-span-2 bg-white rounded-2xl p-4 border border-slate-200 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            ...
            <div className="flex-1 flex gap-2">
              ...
              <button
                onClick={onApply}
                className="rounded-xl px-4 py-2 text-sm font-extrabold bg-[#0B1E3B] text-white"
              >
                Apply
              </button>
              <button
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-200 font-extrabold text-slate-900">
          Messages ({total})
        </div>
        ...
        {loading ? (
          ...
          rows.map((r) => {
            ...
            const data = r.data_obj || safeJson(r.data) || {}; // ✅ prefer enriched if backend adds it
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3 text-xs text-slate-500">
                      ...
                      {data?.donor_name && (
                        <span className="px-2 py-1 rounded-full bg-purple-100 text-purple-700 font-extrabold">
                          {data.donor_name}
                        </span>
                      )}
                    </div>
